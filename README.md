...
# Mod制作方法
0.准备工作：至少需要订阅一个Mod,触发Mod框架的安装（需重启游戏），Mod起作用依赖Mod框架
...
4.将dll保存到ModExample/Assemblies文件夹中（dll不是必须的，简单的修改配置，修改图片或添加游戏内容的Mod不需要编写代码生成dll）  
...
7.Mod开发测试
	...
	如果要测试多个Mod，可以将多个Mod（注意，需包含Mod自身的文件夹），复制到*C:/TerritoryModTestBatch*文件夹中，游戏启动时会自动加载这些Mod.(注意，系统不会检查Mod重复，调试时需注意这个问题）
